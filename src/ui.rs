...
fn render_main_layout(app: &App, area: Rect, buf: &mut Buffer) {
    ...
    let main_vertical =
        Layout::vertical([Constraint::Percentage(70), Constraint::Percentage(30)]).split(outer[0]);
    let top_horizontal =
...
fn render_sidebar(app: &App, area: Rect, buf: &mut Buffer) {
    ...
    let table_count: usize = app
        .db_structure
        .as_ref()
        .map_or(0, |s| s.schemas.iter().map(|sc| sc.tables.len()).sum());
    Paragraph::new(Span::styled(
...
fn build_tree_lines(app: &App) -> (Vec<Line<'static>>, Option<usize>) {
    ...
    for schema in &structure.schemas {
        ...
        let icon = if schema.name == "public" {
            "󰉖 "
        } else {
            "󰉋 "
        };
        ...
        for table in &schema.tables {
            ...
            let is_viewing =
                selected_table.is_some_and(|(s, t)| s == &schema.name && t == &table.name);
...
fn render_graph<F: Fn(u64, u64) -> Color>(
    ...
    Paragraph::new(Line::from(vec![
        ...
        Span::styled(
            format!("{:>4}", current),
            Style::default().fg(color_fn(current, max)).bold(),
        ),
    ]))
    .render(layout[0], buf);
...
fn render_sql_editor(app: &App, area: Rect, buf: &mut Buffer) {
    ...
    let footer = if app.query_executing {
        Line::from(vec![
            ...
            Span::styled(
                format!(" {}ms...", app.query_elapsed_ms().unwrap_or(0)),
